...
internal class ConservativeSilverResolver(
    ...
    private fun resolveMention(
        ...
        if (existingEntityId != null) return MentionResolution(existingEntityId, null, null)
        if (sameName.isNotEmpty()) return MentionResolution(null, null, null)

        val entity = SilverEntity.create()
    ...
    private fun metadataClaims(
        ...
        SilverClaim.create(
            ...
            value = SilverJsonString(mention.name),
            supportingObservationIds = listOf(input.id),
            producer = producer,
            createdAtMillis = createdAtMillis,
        ),
        SilverClaim.create(
            ...
            value = SilverJsonString(mention.type),
            supportingObservationIds = listOf(input.id),
            producer = producer,
            createdAtMillis = createdAtMillis,
        ),
...
private data class Candidate(
    ...
    val scalar: SilverJsonValue? = null,
)
...
private fun List<SilverClaim>.textValues(predicate: String): List<String> = mapNotNull { claim ->
    ...
    (claim.value as? SilverJsonString)?.value
}
...
private fun parseScalar(value: SilverJsonValue?): SilverJsonValue? = when (value) {
    is SilverJsonString, is SilverJsonNumber, is SilverJsonBoolean -> value
    else -> null
}
