...
The Node `source.node.silver-extraction` processor version `5` emits one
...
schema. Literal values describing the subject are scalar attributes; only
references to distinct, independently named entities become relationships.
Semantically invalid self-relationships are retried and never committed. These
settings are private to background refinement; normal chat
